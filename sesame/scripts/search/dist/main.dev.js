                        ...
                        if (JSON.parse(data.sure) != true && sureStatus != true) {
                          ...
                          var generateLINKS = function generateLINKS(dataArray) {
                            var divElements = dataArray.map(function (item) {
                              return "\n                                        <div id=\"".concat(item.id, "-").concat(content.name, "\" class=\"user-link\"  style=\"display:").concat(item.src != "" && item.src.toLowerCase() != "unknown" ? "flex" : "none", "\" onclick=\"").concat(item.id === "discord" ? "copyToClipboard('".concat(item.src, "','").concat(LanguageValues.pop.discord_username, "')") : "window.open('".concat(item.src, "')"), "\">\n                                            <img src=\"").concat(LogosSRC[item.id], "\">\n                                            <div class=\"content\">\n                                                <div class=\"main\">").concat(item.id.replace(/\b\w/g, function (_char) {
