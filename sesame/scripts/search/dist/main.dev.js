              ...
              if (all && particular_query) {
                (function () {
                  ...
                  content.forEach(function (ct) {
                    console.log(keyExistsWithValue(ct, "".concat(particular_query), particular_query_value));
                  ...
                  try {
                    var _loop = function _loop() {
                      ...
                      if (!section_diff[data.id] && !JSON.parse(data.sure)) {
                        ...
                        var _isLastKey = data.id === lastKey;

                        console.log(section_diff_lg, section_diff, "FDSFSDF"); // Set swipe_content and svg_key based on whether it's the last key or not
                        ...
                        setTimeout(function () {
                          console.log('click set to ' + (_isLastKey ? "section-".concat(keys[0]) : "section-".concat(nextKey)));
                      ...
                      if (data.id === "mods") {
                        ...
                        if (JSON.parse(data.sure) != true && sureStatus != true) {
                          ...
                          var generateLINKS = function generateLINKS(dataArray) {
                            var divElements = dataArray.map(function (item) {
                              return "\n                                        <div id=\"".concat(item.id, "-").concat(content.name, "\" class=\"user-link\"  style=\"display:").concat(item.src != "" && item.src.toLowerCase() != "unknown" ? "flex" : "none", "\" onclick=\"").concat(item.id === "discord" ? "navigator.clipboard.writeText('".concat(item.src, "')") : "window.open('".concat(item.src, "')"), "\">\n                                            <img src=\"").concat(LogosSRC[item.id], "\">\n                                            <div class=\"content\">\n                                                <div class=\"main\">").concat(item.id.replace(/\b\w/g, function (_char) {
                        ...
                        if (ships_by_mods[content.mod.toLowerCase()].show != true) {
                          ...
                          console.log(_nextIndex, _keys, "index");
                          ...
                          setTimeout(function () {
                            document.getElementById("".concat(content.mod.replace(/ /g, '-'), "-swipe-scroll")).addEventListener('click', function () {
                              console.log('clicked', _nextIndex);
