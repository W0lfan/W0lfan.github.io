...
function copyToClipboard(content, name) {
  ...
  navigator.clipboard.writeText(url).then(function () {
    DisplayNotif("".concat(name, " ").concat(capitalizeFirstLetter(content.replace(/-/g, ' '))), 5);
      ...
      return regeneratorRuntime.async(function fetchDataFromAPI$(_context5) {
        while (1) {
          switch (_context5.prev = _context5.next) {
            ...
            case 43:
              _context5.next = 45;
              return regeneratorRuntime.awrap(display_ui_by_file('parameters.txt', display_result, document.querySelector('.header .header-search')));
                        ...
                        if (JSON.parse(data.sure) != true && sureStatus != true) {
                          ...
                          var generateLINKS = function generateLINKS(dataArray) {
                            var divElements = dataArray.map(function (item) {
                              return "\n                                        <div id=\"".concat(item.id, "-").concat(content.name, "\" class=\"user-link\"  style=\"display:").concat(item.src != "" && item.src.toLowerCase() != "unknown" ? "flex" : "none", "\" onclick=\"").concat(item.id === "discord" ? "copyToClipboard('".concat(item.src, "','").concat(LanguageValues.pop.discord_username.replace(/'/, '&apos'), "')") : "window.open('".concat(item.src, "')"), "\">\n                                            <img src=\"").concat(LogosSRC[item.id], "\">\n                                            <div class=\"content\">\n                                                <div class=\"main\">").concat(item.id.replace(/\b\w/g, function (_char) {
